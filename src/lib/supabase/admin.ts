...
class AdminQueryBuilder {
  ...
  async execute() {
    try {
      ...
      if (this.method === 'insert') {
        if (this.isUpsert) {
          if (Array.isArray(this.payloadData)) {
            ...
            await this.injectTenantId(this.payloadData)
            let uniqueWhere: any = null
      ...
      console.error('[AdminQueryBuilder] Error executing query:', err)
      return { data: null, error: { message: err.message || String(err) } }
    }
  }

  private async injectTenantId(item: any) {
    if (['flowRun', 'flowRunEvent', 'flow', 'contact', 'conversation', 'appointment', 'broadcast', 'automation'].includes(this.modelName)) {
