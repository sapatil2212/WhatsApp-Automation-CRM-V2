...
class PrismaQueryBuilder {
  ...
  private async injectTenantId(item: any) {
    ...
    if (clientFields && ['flowRun', 'flowRunEvent', 'flow', 'contact', 'conversation'].includes(this.modelName)) {
      if (item.tenantId) return
      
      let userId = item.userId
      if (!userId && item.flowId) {
        const flow = await prisma.flow.findUnique({ where: { id: item.flowId }, select: { userId: true } })
        userId = flow?.userId
      }
      ...
      if (!userId && item.flowRunId) {
        const run = await prisma.flowRun.findUnique({ where: { id: item.flowRunId }, select: { userId: true } })
        userId = run?.userId
      }
...
export function supabaseAdmin(): any {
  return {
    ...
    rpc: async (functionName: string, args: any) => {
      if (functionName === 'increment_flow_execution_count') {
        try {
          await prisma.flow.update({
            where: { id: args.flow_id },
            data: {
